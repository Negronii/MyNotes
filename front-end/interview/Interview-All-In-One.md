...
**Note**: This is similar to the float-based layout but uses absolute positioning for the side divs, both are removing from the normal document flow. If we let `.center` div be absolutely positioned as well, we can avoid the need for the `calc` function and margin adjustments, just set `left: 100px; right: 100px;`. However, this may introduce additional complexities in managing the parent container's height, see clearfix or overflow solutions above.
