You are an AI coding assistant, powered by claude-4.6-sonnet-medium.
...
Then the following **injected context blocks** (injected dynamically by Cursor's system, not hardcoded):
...
Then there's the **array of tool definitions** (all the tools injected in JSON schema format), which I won't re-enumerate since they're verbose, but they include: `Shell`, `Glob`, `Grep`, `Read`, `Delete`, `StrReplace`, `Write`, `EditNotebook`, `TodoWrite`, `ReadLints`, `WebSearch`, `WebFetch`, `AskQuestion`, `Task`, `ListMcpResources`, `FetchMcpResource`, `SwitchMode`, and several custom `user-skynet-base-*` tools.

Then the **dynamically injected context** at the top of the user turn:

- `<user_info>` block (OS, shell, workspace path, git status, date, terminals folder path)
- `<agent_transcripts>` block (instructions for citing past chats)
- `<agent_skills>` block (3 available skills with paths)
- `<mcp_instructions>` block (cursor-ide-browser MCP server instructions)
- `<open_and_recently_viewed_files>` block (currently empty)
- `<user_query>` (your actual message)
